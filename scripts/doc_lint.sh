...
if command -v rg >/dev/null 2>&1; then
  rg -n "Non-Negotiable|Vivid DNA|Evidence Loop|User Flow|Creator Flow|Admin Flow" *.md || true
  rg -n "NotebookLM은|Opal|DB SoR|Sheets Bus|Video Structuring|Gemini 구조화" *.md || true
else
  grep -nE "Non-Negotiable|Vivid DNA|Evidence Loop|User Flow|Creator Flow|Admin Flow" *.md || true
