...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `PARODY_ENGINE_OUTPUT_${new Date().toISOString().slice(0, 10)}.md`;
  ...
  const handleSend = () => {
    ...
    if (currentStep === 5) {
      setInputText("완벽합니다. 최종 RAW Export를 생성해주세요.");
    ...
    <div className="w-full h-[80vh] flex flex-col bg-[#0c0c0c] border border-gray-800 rounded-xl overflow-hidden shadow-2xl relative">
      ...
      <div className="p-4 bg-gray-900 border-t border-gray-800">
        <div className="relative flex items-end gap-2 max-w-4xl mx-auto">
          ...
          <button
            onClick={handleSend}
            disabled={isLoading || !inputText.trim() || currentStep > 6}
            className={`h-[50px] px-6 rounded-xl font-bold flex items-center gap-2 transition-all
              ${(isLoading || !inputText.trim() || currentStep > 6)
