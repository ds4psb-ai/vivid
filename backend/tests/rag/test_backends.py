...
class TestVertexGroundingBackend:
    """VertexGroundingBackend 통합 테스트."""

    @pytest.mark.asyncio
    async def test_retrieve_basic(self):
        """기본 검색 테스트."""
        from app.rag.backends import get_backend
        from app.rag.tier0_vertex_rag import VertexRAGResult, RAGSource

        backend = get_backend("vertex_grounding")
        ...
        mock_result = VertexRAGResult(
            ...
            grounding_sources=[
                {"source": "Google result", "uri": "https://example.com"},
            ],
            confidence=0.85,
            grounded=True,
        )
        ...
        with patch.object(backend, "_get_service") as mock_get_service:
            ...
            results = await backend.retrieve(
                query="test query",
                limit=5,
                config={"corpus_name": "auteur_dna"},
            )

            assert len(results) == 2  # 1 RAG + 1 grounding
            assert results[0].metadata["type"] == "vertex_rag"
            assert results[1].metadata["type"] == "google_search"
    ...
    async def test_retrieve_error_handling(self):
        ...
        with patch.object(backend, "_get_service") as mock_get_service:
            mock_service = AsyncMock()
            mock_service.query = AsyncMock(side_effect=Exception("API Error"))
            mock_get_service.return_value = mock_service
