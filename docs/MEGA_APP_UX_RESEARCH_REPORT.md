...
1. **디자인 불일치 심각**: 메인페이지는 Cinematic OTT 스타일(neon red, deep charcoal, glass morphism)인데, 메가앱은 기본 shadcn/ui 탭 UI 사용
2. **풍부한 디자인 시스템 미활용**: W3C DTCG 2025.10 기반 Oklch 토큰, 18개 앱 컬러, Aurora 배경 등이 메가앱에 적용되지 않음
3. **경쟁사 대비 UX 격차**: Luma AI("가장 아름다운 UX"), Pika(실시간 파라미터 조정) 등 2026 트렌드 미반영
...
| **P0** | 메인페이지 Hero 아래 메가앱 섹션 추가 | 자연스러운 진입점 |
| **P1** | 메가앱별 테마 컬러 적용 | 브랜드 아이덴티티 |
| **P1** | 워크플로우 프로그레스 시각화 | 사용자 방향성 제공 |
| **P2** | 실시간 프리뷰/피드백 패턴 | 전문 도구 경험 |
