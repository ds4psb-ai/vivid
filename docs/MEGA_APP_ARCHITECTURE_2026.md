...
## 6.5 Unified Single Layout (2026 UX 혁신) 🆕

> [!IMPORTANT]
> **2026-01-28 추가**: "4-Panel Split" 대신 "Progressive Flow" 기반 통합 UX
> **검증 점수**: 5/5 핵심 체크포인트 통과 ✅

### 설계 철학

2026 UX 트렌드에 맞춘 **지능형 적응 시스템**:

| 원칙 | 설명 | 기존 vs 신규 |
|------|------|------------|
| **Unified Shell** | 3개 앱이 하나의 Shell 공유 | 앱별 다른 Shell → 1개 Shell |
| **Metadata-Driven** | 설정만 정의하면 UI 자동 생성 | 하드코딩 → Config 기반 |
| **Progressive Deepening** | 단계별 정보량 자동 조절 | 동일 UI → 적응형 UI |
| **Smart Injection** | 필요 데이터 자동 주입 + 누락 감지 | 수동 접근 → 선언적 의존성 |
| **Chain Context** | 전체 워크플로우 데이터 단일 Provider | 끊어진 상태 → 통합 Context |

### 목표 레이아웃

```
┌──────────────────────────────────────────────────────┐
│  Timeline: DNA Lab → Story Engine → Production       │  ← WorkflowProgress
├──────────────────────────────────────────────────────┤
│                                                      │
│  Main (대부분):                  Sidebar:            │
│  ┌────────────────────────────┐  ┌──────────────┐   │
│  │ [현재 단계의 편집 UI]       │  │ Chain Info   │   │  ← Progressive UI
│  │                            │  │ ✓ VPE        │   │    (DNA: 요약)
│  │                            │  │ ✓ AD         │   │    (Story: +미리보기)
│  │                            │  │ ◐ Mirror     │   │    (Prod: 상세)
│  │                            │  │ ○ QC         │   │
...
| # | 혁신 | 컴포넌트 | 효과 |
|:-:|------|---------|------|
| 1 | **Adaptive Layout Engine** | `UnifiedWorkflowShell` | 1개 Shell → 3개 앱 자동 적응 |
| 2 | **Smart Data Injection** | `useRequiredChainData` | 선언적 의존성 + 누락 감지 |
| 3 | **Progressive Deepening** | `UnifiedChainSidebar` | 앱별 정보량 자동 조절 |
| 4 | **Metadata-Driven Workflow** | `WORKFLOW_CONFIGS` | 설정 기반 UI/라우팅/검증 |
| 5 | **Chain Context** | `DimensionChainProvider` | 전역 데이터 흐름 |

### 핵심 파일 구조

```
frontend/src/components/workflow/
├── UnifiedWorkflowShell.tsx   # 적응형 Shell (~200 LOC)
├── UnifiedChainSidebar.tsx    # 모드별 사이드바 (~200 LOC)
├── workflow-configs.ts        # 앱별 설정 정의 (~150 LOC)
├── workflow-metadata.ts       # 단계별 메타데이터 (~200 LOC)
├── AutoGeneratedStepNav.tsx   # 자동 네비게이션 (~100 LOC)
├── MissingDataBanner.tsx      # 누락 데이터 경고 (~60 LOC)
├── types.ts                   # 공통 타입 (~80 LOC)
└── hooks/
    ├── useRequiredChainData.ts    # 스마트 데이터 주입 (~100 LOC)
    ├── useChainDataWithOptimism.ts # Offline-First (~80 LOC)
    └── useWorkflowObservability.ts # 실행 시간 추적 (~100 LOC)
```

### Sidebar 모드별 렌더링

| 앱 | Sidebar Mode | 표시 내용 |
|----|:------------:|----------|
| DNA Lab | `summary` | 아이콘 + 라벨 + 완료 체크 |
| Story Engine | `preview` | 요약 + 클릭 시 확장 미리보기 |
| Production | `detailed` | 전체 데이터 + 메타정보 + 편집 링크 |

### 데이터 흐름 (Smart Injection)

```typescript
// 기존: 수동 접근
const chainCtx = useDimensionChainOptional();
const vpeData = chainCtx?.chainData?.vpe; // null 체크 필요
...
| Phase | 작업 | 시간 | 우선순위 |
|:-----:|------|:----:|:--------:|
| 0 | Provider Wiring (`layout.tsx`) | 10분 | P0 |
| 1 | Adaptive Layout Engine | 30분 | P0 |
| 2 | Smart Data Injection | 20분 | P1 |
| 3 | Progressive Deepening UI | 20분 | P1 |
| 4 | Metadata-Driven Workflow | 20분 | P1 |
| 5 | Optimistic State Persistence | 15분 | P2 |
| 6 | Workflow Observability | 15분 | P2 |
| 7 | Page Migration | 30분 | P1 |
| | **총** | **~2시간 40분** | |
...
| # | 질문 | 결과 |
|:-:|------|:----:|
| 1 | UnifiedWorkflowShell 1개? (3개 앱이 공용) | ✅ |
| 2 | WORKFLOW_CONFIGS 메타데이터 기반? | ✅ |
| 3 | AdaptiveChainSummary mode별 렌더? | ✅ |
| 4 | AutoGeneratedActions config 기반? | ✅ |
| 5 | useRequiredChainData smart injection? | ✅ |
...
| **Phase 6.5** | Unified Single Layout | 🆕 계획 | 2시간 40분 |
...
*Version 2.1 Updated: 2026-01-28 | Section 6.5 Unified Single Layout Added*
