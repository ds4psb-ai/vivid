...
**목표**: PD/작가/창작자에게 최고 가치를 제공하는 워크플로우(UI) 요구사항 정리  
**Status**: Legacy (Node Canvas는 내부 그래프 전용, UI는 Train Workflow/Dimension 중심)
...
- 템플릿 기반 워크플로우(열차)로 **빠른 기획 → 미리보기 → 생성**을 실현한다.
- 역할/흐름 상세 정본: `08_PIPELINES_AND_USER_FLOWS.md`
...
- **Template-first**: 카드 클릭 → 즉시 워크플로우 시작
...
세부 단계는 `08_PIPELINES_AND_USER_FLOWS.md`에 정본화한다.
요약 흐름:
- Creator: 템플릿 → 워크플로우(열차) → 캡슐 실행 → 프리뷰 → 생성
...
- 템플릿 카드 갤러리 + 1-클릭 워크플로우 시작
- 열차 연결 고리 선택 UI (3개 추천 중 택1)
- 캡슐 실행 (SSE 스트리밍)
- 프리뷰 패널 (요약/팔레트/씬 힌트)
...
- 역할 정의/증명 레이어: `08_PIPELINES_AND_USER_FLOWS.md`, `15_CREBIT_ARCHITECTURE_EVOLUTION_CODEX.md`
- 영상 구조화 기준: `19_VIDEO_UNDERSTANDING_PIPELINE_CODEX.md`
...
- Template-first IA
- Empty state: “Create First Workflow” CTA
- 노드 그래프는 UI에 노출하지 않음 (내부 오케스트레이션 전용)
