# Crebit Execution Plan v1 (Post-Research, CODEX-aligned)

**Date**: 2025-12-24 (Updated: 2025-12-28)  
**Status**: Phase 1-4 Complete (100% Pipeline)  
**Basis**: `20_CREBIT_ARCHITECTURE_EVOLUTION_CODEX.md`  
**Goal**: Preserve Crebit philosophy while sequencing the work to avoid rework.
...
**Dev Tasks**:
- [x] Extend Sheets ingest to include Notebook Library (manual first, automate later).
- [x] Add Notebook Library tables in DB (metadata + outputs).
- [x] Add promotion rules for Notebook outputs -> Pattern Library/Trace.
...
**Dev Tasks**:
- [x] Add input contract validation in capsule run.
- [x] Capture upstream context snapshot per run.
- [x] Enforce evidence refs from DB SoR only.
...
**Dev Tasks**:
- [x] Refactor Sidebar + TopBar.
- [x] Empty-state seed graph UI.
- [x] Node status states + preview panel integration.
...
**Dev Tasks**:
- [x] Define evidence reward function for GA/RL.
- [x] Add template version promotion rules.
- [x] Persist learning history for auditability.
...
**Dev Tasks**:
- [x] Implement credit ledger + balance.
- [x] Add affiliate reward gating (verified email + first run).
- [x] Surface token usage and cost per run in UI.
