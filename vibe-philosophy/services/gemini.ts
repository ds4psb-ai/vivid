...
<rules>
1. 빈 문자열(""), 빈 배열([]), 빈 객체({})인 필드들을 **반드시 모두** 채울 것
2. 이미 값이 있는 필드는 절대 수정하지 말 것
3. 대화에 직접 근거가 없어도, 드러난 성격/말투/패턴에서 합리적으로 유추하여 채울 것
4. 빈 필드를 남기지 말 것 - 모든 필드에 의미 있는 값 필수
...
<output>
완성된 전체 페르소나 JSON을 반환하세요. 기존 값은 유지하고 빈 필드는 **전부** 채우세요.
빈 문자열(""), 빈 배열([])이 하나도 없어야 합니다.
</output>
`;
